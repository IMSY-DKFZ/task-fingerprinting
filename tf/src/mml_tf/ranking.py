...
class BootstrapRanking:
    def __init__(
        ...
        """
        Creates a bootstrap ranking similar to challengeR (see https://www.rankings-reloaded.de/). You can access the
        ranking or derived statistics via properties of this class. Code originally from Jan Sellner as part of the
        IMSY htc repository at https://github.com/IMSY-DKFZ/htc/blob/main/htc/evaluation/ranking.py.

        For each bootstrap, metric values are sampled with replacement for each task and algorithm and then a ranking
        is performed. The same samples are used across algorithms.

        Note: For reproducible results, set a numpy seed before using this class.

        :param pd.DataFrame data: Dataframe with the metric data.
        :param Optional[str] task: Name of the task column in the dataframe. A ranking is performed separately per task.
            Set to None if you do not want to analyze separate tasks (a dummy task column will be added to the tables).
        :param str algorithm: Name of the algorithm column in the dataframe. There must be at least two algorithms
            which you want to compare against each other.
        :param str case: Name of the column which contains the sample names (e.g. label name or subject name).
        :param str value: Name of the column which contains the metric data. There must be one value per sample.
        :param int n_bootstraps: Number of bootstraps to perform.
        :param bool smaller_better: If True, smaller numbers indicate a better algorithm performance.
        :param bool use_median: If True, aggregates bootstrapped values by median instead of mean before ranking.
        """
        self.data = data
        self.task = task
    ...
    def bootstraps(self) -> pd.DataFrame:
        ...
        for task_name in df_selection[self.task].unique():
            ...
            for algorithm_name in algorithms:
                ...
                # Bootstrap selection of the sample values and mean/median score per bootstrap
                if self.use_median:
                    aggregated = np.median(values[bootstrap_indices], axis=0)
                else:
                    aggregated = np.mean(values[bootstrap_indices], axis=0)
                task_data.append(aggregated)

            # Rankings for this task
            task_data = np.stack(task_data)
            rankings = rankdata(task_data, axis=0, method="average")
